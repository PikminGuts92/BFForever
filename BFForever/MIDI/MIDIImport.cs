...
namespace BFForever.MIDI
{
    public class MIDIImport : ChartImport
    {
        ...
        private static Regex _eventRegex = new Regex(@"\[prc_[a-zA-Z0-9_]+\]"); // [event_name]
        private static Regex _sectionRegex = new Regex(@"\[section[\s]+[a-zA-Z0-9_]+\]"); // [section event_name]
        private static Regex _sectionEventNameRegex = new Regex(@"[a-zA-Z0-9_]+\]"); // event_name]
        
        // Auto-generated code!
        private static readonly IReadOnlyDictionary<string, string> _practiceSections = new Dictionary<string, string>()
        {
            { "prc_a", "A section" },
            { "prc_a1", "A section 1" },
            { "prc_a2", "A section 2" },
        ...
        private List<Riff.TextEvent> ExportSectionEntries()
        {
            List<Riff.TextEvent> entries = new List<Riff.TextEvent>();
            var eventsTrack = _midiFile.Events.FirstOrDefault(x => x[0].ToString().Contains("EVENTS"));
            if (eventsTrack == null) return entries;

            int sixteenth = _midiFile.DeltaTicksPerQuarterNote >> 2; // 1/16 note

            foreach (var ev in eventsTrack.Where(x => x is MetaEvent && ((MetaEvent)x).MetaEventType == MetaEventType.TextEvent).Select(x => x as NAudio.Midi.TextEvent))
            {
                string sectionName;
                if (!GetSectionName(ev.Text, out sectionName)) continue;
                
                Riff.TextEvent entry = new Riff.TextEvent()
                {
                    Start = (float)GetRealTime(ev.AbsoluteTime),
                    End = (float)GetRealTime(ev.AbsoluteTime + sixteenth),
        ...
        private bool GetSectionName(string keyword, out string sectionName)
        {
            // Pre-RB3 practice section
            if (_sectionRegex.IsMatch(keyword))
            {
                string key = _sectionEventNameRegex.Match(keyword).Value.Replace("]", string.Empty);

                if (!_practiceSections.TryGetValue("prc_" + key, out sectionName))
                    sectionName = key; // System default?
                
                return true;
            }

            // Post-RB3 practice section
            if (_eventRegex.IsMatch(keyword))
            {
                string key = _eventRegex.Match(keyword).Value.Replace("[", string.Empty).Replace("]", string.Empty);

                return _practiceSections.TryGetValue(key, out sectionName);
            }
